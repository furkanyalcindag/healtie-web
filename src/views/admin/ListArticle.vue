<template>
  <CRow>
          ...
          <CRow>
            ...
            <CCol lg="9" class="text-right mt-3">
              <CButton
                ...
                class="float-end"
                shape="rounded-pill"
                @click="showModal('addArticleModal')"
                >Ekle</CButton
              >
            </CCol>
          </CRow>
    ...
    <CModal
      backdrop="static"
      size="lg"
      :visible="openedModals.addArticleModal"
    ...
    >
      ...
      <CModalBody>
        <CForm
          ...
          @submit.prevent="
            isAbleToPushButton
              ? submitToAPI($event, 'addArticleModal', addedItem.data)
              : null
          "
          needs-validation
          novalidate
        ...
        >
          ...
          <CModalFooter class="pe-0">
            <CButton
              color="secondary"
              @click="closeModal('addArticleModal', true)"
              >İptal</CButton
            >
            <CButton
              color="success"
...
export default {
  ...
  created() {
    this.getCategories(this.articleTable.serverOptions)
  },
  methods: {
    ...mapActions({
    ...
    async addCategory(data) {
      ...
      if (response === true) {
        ...
        this.getCategories(this.articleTable.serverOptions)
        this.closeModal('addArticleModal', true)
      } else {
        new Toast(
